...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, dw2, db2 = affine_relu_backward(dloss, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,reg=0.0,
        ...
        for i in range(len(hidden_dims)):
            ...
            wStr = 'W' + str(index1)
            bStr = 'b' + str(index1)
            self.params[wStr] = weight_scale * np.random.randn(d, m)
            self.params[bStr] = np.zeros(m)
        index1 += 1  
        d = m
        self.params['W' + str(self.num_layers)] = weight_scale * np.random.randn(d, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            ...
            x_curr, cache = affine_relu_forward(x_curr, w, b)
            caches.append(cache)
            
        w = self.params['W'+str(self.num_layers)]
        b = self.params['b'+str(self.num_layers)]
        scores, cache = affine_forward(x_curr ,w,b)
        caches.append(cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        def computeAllRegs():
            ...
            for i in range(1, self.num_layers + 1):
                wc = self.params['W' + str(i)]
                toReturn += np.sum(wc * wc)
            return self.reg * 0.5 * toReturn
